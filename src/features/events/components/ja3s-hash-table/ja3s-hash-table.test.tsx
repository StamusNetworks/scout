...
import { JA3SHashTable } from './ja3s-hash-table';
...
const createTestRouter = () =>
  createRouter({
    ...
    history: createMemoryHistory({ initialEntries: ['/beaconing/ja3s'] }),
  });
...
const renderTable = async (overrides?: Partial<typeof defaultProps>) => {
  ...
  return renderWithProviders(<JA3SHashTable {...props} />, {
...
describe('JA3SHashTable', () => {
  ...
  it('renders empty state when no results', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('No beaconing JA3s hashes found'),
      ).toBeInTheDocument();
    });
  });
});
