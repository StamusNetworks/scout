...
const createTestRouter = () =>
  createRouter({
    ...
    history: createMemoryHistory({
      initialEntries: ['/beaconing/ips/203.0.113.42'],
    }),
  });
...
describe('BeaconingIpDetails', () => {
  it('renders IP value and score when data loads', async () => {
    server.use(
      http.get(baseUrl + '/appliances/es_beaconing_events/', () =>
        HttpResponse.json({
          count: 1,
    ...
    await renderWithProviders(<BeaconingIpDetails ip="203.0.113.42" />, {
  ...
  it('renders empty state when no results', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('No beaconing data found for this IP.'),
      ).toBeInTheDocument();
    });
  });
});
