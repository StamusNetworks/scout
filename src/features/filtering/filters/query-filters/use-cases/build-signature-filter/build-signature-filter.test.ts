...
import { buildSignatureFilters } from './build-signature-filter';
...
describe('buildSignatureFilters', () => {
  it('should comma-separate multiple content filters', () => {
    ...
    const result = buildSignatureFilters(filters);
  ...
  it('should return undefined when no signature filters exist', () => {
    expect(buildSignatureFilters([])).toBeUndefined();
  });
});
