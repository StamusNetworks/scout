...
export const Navigation = ({ menu, className }: MenuProps) => {
  ...
  return (
    <NavigationMenu
      className={cn(
        `bg-menu relative h-full flex-none shrink-0 overflow-hidden border-r px-0 transition-all duration-300 ease-in-out`,
        isOpen ? 'min-w-60' : 'w-0 p-0',
        className,
      )}
    >
      <ScrollArea className="flex h-full min-h-screen w-full flex-none shrink-0 flex-col justify-between overflow-clip px-2 [&>div>div]:h-full">
