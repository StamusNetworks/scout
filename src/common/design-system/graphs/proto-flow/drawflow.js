/* eslint-disable */
/* Based on https://github.com/jerosoler/Drawflow */
import { clone } from 'ramda';

export default class Drawflow {
  constructor(container, prefix, render = null) {
  ...
  start() {
    ...
    return; // Disable canvas dragging
        ...
        if (output_id !== input_id && input_class !== false) {
          if (
            ...
            this.connection_ele.classList.add(
              'node_in_' + input_id + this.namespace,
            );
            this.connection_ele.classList.add(
              'node_out_' + output_id + this.namespace,
            );
  ...
  addConnection(id_output, id_input, output_class, input_class) {
    ...
    if (nodeOneModule === nodeTwoModule) {
      ...
      if (exist === false) {
        //Create Connection
        this.drawflow.drawflow[nodeOneModule].data[
          id_output + this.namespace
        ].outputs[output_class].connections.push({
          node: id_input + this.namespace.toString(),
          output: input_class,
        });
        this.drawflow.drawflow[nodeOneModule].data[
          id_input + this.namespace
        ].inputs[input_class].connections.push({
          node: id_output + this.namespace.toString(),
          input: output_class,
        });
        ...
        if (this.module === nodeOneModule) {
          ...
          // path.innerHTML = 'a';
          connection.classList.add('connection');
          connection.classList.add('node_in_node-' + id_input + this.namespace);
          connection.classList.add(
            'node_out_node-' + id_output + this.namespace,
          );
          connection.classList.add(output_class);
          connection.classList.add(input_class);
          connection.appendChild(path);
          this.precanvas.appendChild(connection);
          this.updateConnectionNodes('node-' + id_output + this.namespace);
          this.updateConnectionNodes('node-' + id_input + this.namespace);
        }
  ...
  updateConnectionNodes(id) {
    ...
    Object.keys(elemsOut).map(function (item, index) {
      if (elemsOut[item].querySelector('.point') === null) {
        ...
        if (typeof elemtsearchOut === 'undefined') {
          console.log('item:', item);
          console.log('elemtsearchId_out: ', id);
          return;
        }
  ...
  getNodeFromId(id) {
    ...
    return clone(this.drawflow.drawflow[moduleName].data[id + this.namespace]);
  }
  ...
  addNode(
    ...
    node.innerHTML = '';
    node.setAttribute('id', 'node-' + newNodeId + this.namespace);
    ...
    if (typenode === false) {
      ...
      content.setAttribute('title', html);
    ...
    this.drawflow.drawflow[this.module].data[newNodeId + this.namespace] = json;
    this.dispatch('nodeCreated', newNodeId + this.namespace);
    if (!this.useuuid) {
      this.nodeId++;
    }
    return newNodeId + this.namespace;
  }
    ...
    Object.keys(dataNode.inputs).map(function (input_item, index) {
      ...
      Object.keys(dataNode.inputs[input_item].connections).map(
        function (output_item, index) {
          ...
          connection.classList.add('connection');
          connection.classList.add(
            'node_in_node-' + dataNode.id + this.namespace,
          );
          connection.classList.add(
            'node_out_node-' +
  ...
  addNodeOutput(id) {
    ...
    if (this.module === moduleName) {
      ...
      const parent = document.querySelector(
        '#node-' + id + this.namespace + ' .outputs',
      );
  ...
  removeNodeInput(id, input_class) {
    ...
    if (this.module === moduleName) {
      document
        .querySelector(
          '#node-' + id + this.namespace + ' .inputs .input.' + input_class,
        )
        .remove();
    }
    const removeInputs = [];
    ...
    if (this.module === moduleName) {
      const eles = document.querySelectorAll(
        '#node-' + id + this.namespace + ' .inputs .input',
      );
      eles.forEach((item, i) => {
  ...
  removeNodeOutput(id, output_class) {
    ...
    if (this.module === moduleName) {
      document
        .querySelector(
          '#node-' + id + this.namespace + ' .outputs .output.' + output_class,
        )
        .remove();
    }
    const removeOutputs = [];
    ...
    if (this.module === moduleName) {
      const eles = document.querySelectorAll(
        '#node-' + id + this.namespace + ' .outputs .output',
      );
      eles.forEach((item, i) => {
  ...
  removeConnectionNodeId(id) {
    const idSearchIn = 'node_in_' + id + this.namespace;
    const idSearchOut = 'node_out_' + id + this.namespace;
  ...
  export() {
    const dataExport = clone(this.drawflow);
